...
def process_folder(
    input_folder: str, output_folder: str, config: ExtractConfig, out_formats: set[str]
):
    ...
    formatters = {fmt: FormatterFactory.get_formatter(fmt) for fmt in out_formats}
...
def main():
    ...
    parser.add_argument("--prompt_start_token", help="Start token for essay prompt extraction")
    parser.add_argument("--prompt_end_token", help="End token for essay prompt extraction")
    ...
    if invalid_formats:
        parser.error(
            f"Invalid output format(s): {', '.join(invalid_formats)}. Valid formats are: {', '.join(valid_formats)}"
        )

    config = ExtractConfig(
        prompt_start_token=args.prompt_start_token,
