...
class FormatterFactory:
    _formatters: Dict[str, tuple[Type[BaseFormatter], FormatterConfig]] = {
        "json": (
            JsonFormatter,
            FormatterConfig(extension=".json", output_subfolder="json"),
        ),
        "html": (
            HtmlFormatter,
            FormatterConfig(extension=".html", output_subfolder="html"),
        ),
        "xml": (
            XmlFormatter,
            FormatterConfig(extension=".xml", output_subfolder="xml"),
        ),
    }
    ...
    def get_formatter(cls, name: str) -> BaseFormatter:
        ...
        if name not in cls._formatters:
            raise ValueError(
                f"Unknown formatter: {name}. Valid formatters are: {', '.join(cls._formatters.keys())}"
            )
